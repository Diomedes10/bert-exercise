...
def process_sst(raw_dir, output_file):
    ...
    print(f"Data head after merge:\n{data.head()}")

    print("Ensuring sentences are strings...")
    sentences['sentence'] = sentences['sentence'].astype(str)
    dictionary['phrase'] = dictionary['phrase'].astype(str)

    print("Mapping phrase ids to sentence ids...")
    # Mapping phrase ids to sentence ids
    phrase_to_sentence = {}
    for _, row in sentences.iterrows():
        sentence_id = row['sentence_id']
        sentence = row['sentence']
        for _, dict_row in dictionary.iterrows():
            phrase = dict_row['phrase']
            if phrase in sentence:
                phrase_id = dict_row['phrase_id']
                phrase_to_sentence[phrase_id] = sentence_id

    print("Adding sentence_id to the data...")
    # Add sentence_id to the data based on phrase_id mapping
    data['sentence_id'] = data['phrase_id'].map(phrase_to_sentence)
    print(f"After adding sentence_id: {data.shape}")
    print(f"Data head after adding sentence_id:\n{data.head()}")
